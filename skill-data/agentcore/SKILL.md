---
name: agentcore
description: Run web-action on AWS Bedrock AgentCore cloud browsers. Use when the user wants to use AgentCore, run browser automation on AWS, use a cloud browser with AWS credentials, or needs a managed browser session backed by AWS infrastructure. Triggers include "use agentcore", "run on AWS", "cloud browser with AWS", "bedrock browser", "agentcore session", or any task requiring AWS-hosted browser automation.
allowed-tools: Bash(web-action:*), Bash(npx web-action:*)
---

# AWS Bedrock AgentCore

Run web-action on cloud browser sessions hosted by AWS Bedrock AgentCore. All standard web-action commands work identically; the only difference is where the browser runs.
...
web-action -p agentcore open https://example.com
...
```bash
# First run: log in
AGENTCORE_PROFILE_ID=my-app web-action -p agentcore open https://app.example.com/login
web-action snapshot -i
...
AGENTCORE_PROFILE_ID=my-app web-action -p agentcore open https://app.example.com/dashboard
```
...
# Explicit region
AGENTCORE_REGION=eu-west-1 web-action -p agentcore open https://example.com
```
...
AWS_PROFILE=my-profile web-action -p agentcore open https://example.com
