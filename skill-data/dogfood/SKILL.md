...
Always use `web-action` directly -- never `npx web-action`. The direct binary uses the fast Rust client. `npx` routes through Node.js and is significantly slower.
...
```bash
web-action --session {SESSION} open {TARGET_URL}
web-action --session {SESSION} wait --load networkidle
```
...
```bash
web-action --session {SESSION} snapshot -i
# Identify login form refs, fill credentials
web-action --session {SESSION} fill @e1 "{EMAIL}"
web-action --session {SESSION} fill @e2 "{PASSWORD}"
web-action --session {SESSION} click @e3
web-action --session {SESSION} wait --load networkidle
```
...
```bash
web-action --session {SESSION} state save {OUTPUT_DIR}/auth-state.json
```
...
```bash
web-action --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/initial.png
web-action --session {SESSION} snapshot -i
```
...
```bash
web-action --session {SESSION} snapshot -i
web-action --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/{page-name}.png
web-action --session {SESSION} errors
web-action --session {SESSION} console
```
...
```bash
web-action --session {SESSION} record start {OUTPUT_DIR}/videos/issue-{NNN}-repro.webm
```
...
```bash
web-action --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-1.png
sleep 1
# Perform action (click, fill, etc.)
sleep 1
web-action --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-2.png
sleep 1
...
```bash
sleep 2
web-action --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}-result.png
```

4. **Stop the video:**

```bash
web-action --session {SESSION} record stop
```
...
```bash
web-action --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}.png
```
...
```bash
web-action --session {SESSION} close
```
...
- **Be efficient with commands.** Batch multiple `web-action` commands in a single shell call when they are independent (e.g., `web-action ... screenshot ... && web-action ... console`). Use `web-action --session {SESSION} scroll down 300` for scrolling -- do not use `key` or `evaluate` to scroll.
