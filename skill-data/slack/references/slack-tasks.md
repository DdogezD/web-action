...
2. **Check Activity Tab**
   - Take snapshot: `web-action snapshot -i`
   - Look for Activity tab ref (usually `@e14`)
   - Click: `web-action click @e14`
   - Wait: `web-action wait 1000`
   - If you see "You've read all the unreads", you have no unread messages
   - Screenshot: `web-action screenshot activity.png`
...
2. **Look for "Channels" treeitem**
   ...
   - Screenshot: `web-action screenshot all-channels.png`

3. **Scroll through sidebar**
   ```bash
   # If the list is long, scroll within the sidebar
   web-action scroll down 500 --selector ".p-sidebar"
   web-action screenshot channels-page-2.png
   ```

4. **Parse snapshot for channel list**
   ```bash
   web-action snapshot --json > channels.json
...
2. **Enter search term**
   ...
   web-action fill @e_search_input "your keyword"
...
3. **Capture results**
   ...
   web-action snapshot -i > search-snapshot.txt
   ```
...
2. **Check channel info**
   - Screenshot channel details: `web-action screenshot channel-header.png`
   - Look for member count, description, topic

3. **View messages**
   ```bash
   # Jump to recent/unread
   web-action press j  # Jump to unread in Slack
   web-action wait 500
   web-action screenshot recent-messages.png
   ```

4. **Scroll to see more**
   ```bash
   web-action scroll down 500
   web-action screenshot more-messages.png
   ```

5. **Check threads**
   - Click on messages with thread indicators
   - View replies in thread view
   - Screenshot: `web-action screenshot thread.png`
...
1. **Navigate to relevant channel or DM**
   ```bash
   web-action click @e_conversation_ref
   web-action wait 1000
   ```

2. **Take snapshot with context**
   ```bash
   web-action snapshot --json > conversation.json
   ```
...
5. **Screenshot key messages**
   ```bash
   web-action screenshot important-message.png
   web-action screenshot --annotate annotated-message.png
   ```
...
2. **Click reaction button to expand**
   ```bash
   web-action click @e_reaction_button
   web-action wait 500
   ```

3. **Capture reaction details**
   ```bash
   web-action screenshot reactions.png
...
2. **Click Pins tab**
   ...
   - Click it: `web-action click @e_pins_tab`
   - Wait: `web-action wait 500`

3. **View pinned messages**
   ```bash
   web-action screenshot pins.png
   web-action snapshot -i > pins-snapshot.txt
   ```

4. **Review each pin**
   - Click pin to see context
   - Note who pinned it, when, and why
   - Screenshot: `web-action screenshot pin-detail.png`
...
1. **Check it's visible**
   ...
   web-action screenshot current-state.png
...
2. **Try expanding/scrolling**
   ...
   web-action scroll down 300 --selector ".p-sidebar"
   web-action snapshot -i
   ```
