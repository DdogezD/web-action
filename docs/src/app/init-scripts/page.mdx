...
agent-browser --enable react-devtools,other-feature open http://localhost:3000
```

`react-devtools` installs the React DevTools hook before application code runs. It is required for `react tree`, `react inspect`, `react renders`, and `react suspense`.
