...
`record start` can also navigate immediately:

```bash
agent-browser record start ./demo.webm https://example.com
```

If no URL is provided, recording starts from the current page. The recording context preserves cookies and localStorage from the active session.
...
<table>
  ...
  <tbody>
    ...
    <tr><td>State</td><td>Preserves cookies and localStorage from the active session</td></tr>
  </tbody>
</table>
