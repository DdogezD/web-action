...
```bash
./scripts/windows-debug/run.sh "cd C:\web-action && cargo test --manifest-path cli\Cargo.toml"
```

Run e2e tests on Windows:

```bash
./scripts/windows-debug/run.sh "cd C:\web-action && cargo test e2e --manifest-path cli\Cargo.toml -- --ignored --test-threads=1"
```
...
The repo lives at `C:\web-action` on the instance. Rust, Git, and Chrome are pre-installed. The `run.sh` wrapper automatically adds cargo and git to PATH.
