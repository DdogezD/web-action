    ...
    pub fn format(&self) -> String {
        match self {
            ...
            ParseError::InvalidValue { message, usage } => {
                format!("{}\nUsage: web-action {}", message, usage)
            }
    ...
    match cmd {
            ...
            match sub {
                Some("save") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "web-action auth save <name> --url <url> --username <user> --password <pass>",
                    })?;
                    ...
                    let url_val = url.ok_or_else(|| ParseError::MissingArguments {
                        context: "auth save".to_string(),
                        usage: "web-action auth save <name> --url <url> --username <user> --password <pass> [--password-stdin]",
                    })?;
                ...
                Some("login") => {
                    const AUTH_LOGIN_USAGE: &str = "web-action auth login <name> [--credential-provider <plugin>] [--item <ref>] [--url <url>] [--username-selector <s>] [--password-selector <s>] [--submit-selector <s>]";
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                ...
                Some("delete") | Some("remove") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "web-action auth delete <name>",
                    })?;
                ...
                Some("show") => {
                    let name = rest.get(1).ok_or_else(|| ParseError::MissingArguments {
                        ...
                        usage: "web-action auth show <name>",
                    })?;
        ...
        "confirm" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "web-action confirm <confirmation-id>",
            })?;
        ...
        "deny" => {
            let cid = rest.first().ok_or_else(|| ParseError::MissingArguments {
                ...
                usage: "web-action deny <confirmation-id>",
            })?;
...
mod tests {
    ...
    // === Default timeout (WEB_ACTION_DEFAULT_TIMEOUT) tests ===
