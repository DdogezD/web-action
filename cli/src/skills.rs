...
///
/// Resolution order:
/// 1. WEB_ACTION_SKILLS_DIR env var (points directly at a single directory)
...
fn find_package_root() -> Option<PathBuf> {
    if let Ok(exe) = env::current_exe() {
        ...
        if let Some(parent) = exe.parent() {
            // npm install layout: bin/web-action-* -> ../
...
fn find_skills_dirs() -> Vec<PathBuf> {
    ...
    if let Ok(dir) = env::var("WEB_ACTION_SKILLS_DIR") {
    ...
    if targets.is_empty() {
        if json_mode {
            println!(
                ...
                serde_json::to_string(&json!({
                    "success": false,
                    "error": "No skill name provided. Usage: web-action skills get <name>",
                }))
                .unwrap_or_default()
            );
        } else {
            eprintln!(
                "{} No skill name provided. Usage: web-action skills get <name>",
    ...
    if skills_dirs.is_empty() {
        if json_mode {
            println!(
                ...
                serde_json::to_string(&json!({
                    "success": false,
                    "error": "Skills directory not found. Set WEB_ACTION_SKILLS_DIR or reinstall via npm.",
                }))
                .unwrap_or_default()
            );
        } else {
            eprintln!(
                "{} Skills directory not found. Set WEB_ACTION_SKILLS_DIR or reinstall via npm.",
                color::error_indicator()
            );
        }
        exit(1);
