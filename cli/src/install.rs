...
pub fn find_installed_chrome() -> Option<PathBuf> {
    ...
    let debug = std::env::var("WEB_ACTION_DEBUG").is_ok();
...
fn http_client() -> Result<reqwest::Client, String> {
    reqwest::Client::builder()
        .user_agent(format!("web-action/{}", env!("CARGO_PKG_VERSION")))
...
pub fn run_install(with_deps: bool) {
    if cfg!(all(target_os = "linux", target_arch = "aarch64")) {
        ...
        eprintln!("  Then use: web-action --executable-path /usr/bin/chromium");
        exit(1);
    }
    ...
    if is_linux {
        if with_deps {
            ...
            println!("  web-action install --with-deps");
            println!();
        }
    }
...
mod tests {
    ...
    async fn http_client_sends_user_agent() {
        ...
        let expected_ua = format!("web-action/{}", env!("CARGO_PKG_VERSION"));
        assert!(
