...
pub(crate) const INTERNAL_DAEMON_SHUTDOWN_ACTION: &str = "__web_action_internal_shutdown";
...
/// Priority: WEB_ACTION_SOCKET_DIR > XDG_RUNTIME_DIR > ~/.web-action > tmpdir
pub fn get_socket_dir() -> PathBuf {
    // 1. Explicit override (ignore empty string)
    let base = if let Ok(dir) = env::var("WEB_ACTION_SOCKET_DIR") {
        if !dir.is_empty() {
            PathBuf::from(dir)
        } else if let Ok(runtime_dir) = env::var("XDG_RUNTIME_DIR") {
            if !runtime_dir.is_empty() {
                PathBuf::from(runtime_dir).join("web-action")
            } else if let Some(home) = dirs::home_dir() {
                home.join(".web-action")
            } else {
                env::temp_dir().join("web-action")
            }
        } else if let Some(home) = dirs::home_dir() {
            home.join(".web-action")
        } else {
...
fn apply_daemon_env(cmd: &mut Command, session: &str, opts: &DaemonOptions) {
    cmd.env("WEB_ACTION_DAEMON", "1")
        .env("WEB_ACTION_SESSION", session);

    if opts.headed {
        cmd.env("WEB_ACTION_HEADED", "1");
    }
    if opts.debug {
        cmd.env("WEB_ACTION_DEBUG", "1");
    }
    if let Some(path) = opts.executable_path {
        cmd.env("WEB_ACTION_EXECUTABLE_PATH", path);
    }
    if !opts.extensions.is_empty() {
        cmd.env("WEB_ACTION_EXTENSIONS", opts.extensions.join(","));
    }
    if !opts.init_scripts.is_empty() {
        cmd.env("WEB_ACTION_INIT_SCRIPTS", opts.init_scripts.join(","));
    }
    if !opts.enable.is_empty() {
        cmd.env("WEB_ACTION_ENABLE", opts.enable.join(","));
    }
    if let Some(a) = opts.args {
        cmd.env("WEB_ACTION_ARGS", a);
    }
    if let Some(ua) = opts.user_agent {
        cmd.env("WEB_ACTION_USER_AGENT", ua);
    }
    if let Some(p) = opts.proxy {
        cmd.env("WEB_ACTION_PROXY", p);
    }
    if let Some(pb) = opts.proxy_bypass {
        cmd.env("WEB_ACTION_PROXY_BYPASS", pb);
    }
    if let Some(pu) = opts.proxy_username {
        cmd.env("WEB_ACTION_PROXY_USERNAME", pu);
    }
    if let Some(pp) = opts.proxy_password {
        cmd.env("WEB_ACTION_PROXY_PASSWORD", pp);
    }
    if opts.ignore_https_errors {
        cmd.env("WEB_ACTION_IGNORE_HTTPS_ERRORS", "1");
    }
    if opts.allow_file_access {
        cmd.env("WEB_ACTION_ALLOW_FILE_ACCESS", "1");
    }
    cmd.env(
        "WEB_ACTION_HIDE_SCROLLBARS",
        if opts.hide_scrollbars { "1" } else { "0" },
    );
    if let Some(prof) = opts.profile {
        cmd.env("WEB_ACTION_PROFILE", prof);
    }
    if let Some(st) = opts.state {
        cmd.env("WEB_ACTION_STATE", st);
    }
    if let Some(p) = opts.provider {
        cmd.env("WEB_ACTION_PROVIDER", p);
    }
    if let Some(d) = opts.device {
        cmd.env("WEB_ACTION_IOS_DEVICE", d);
    }
    if let Some(sn) = opts.session_name {
        cmd.env("WEB_ACTION_SESSION_NAME", sn);
    }
    if let Some(policy) = opts.restore_save {
        cmd.env("WEB_ACTION_RESTORE_SAVE", policy);
    }
    if let Some(check) = opts.restore_check_url {
        cmd.env("WEB_ACTION_RESTORE_CHECK_URL", check);
    }
    if let Some(check) = opts.restore_check_text {
        cmd.env("WEB_ACTION_RESTORE_CHECK_TEXT", check);
    }
    if let Some(check) = opts.restore_check_fn {
        cmd.env("WEB_ACTION_RESTORE_CHECK_FN", check);
    }
    if let Some(dp) = opts.download_path {
        cmd.env("WEB_ACTION_DOWNLOAD_PATH", dp);
    }
    if let Some(ad) = opts.allowed_domains {
        cmd.env("WEB_ACTION_ALLOWED_DOMAINS", ad.join(","));
    }
    if let Some(ap) = opts.action_policy {
        cmd.env("WEB_ACTION_ACTION_POLICY", ap);
    }
    if let Some(ca) = opts.confirm_actions {
        cmd.env("WEB_ACTION_CONFIRM_ACTIONS", ca);
    }
    if let Some(engine) = opts.engine {
        cmd.env("WEB_ACTION_ENGINE", engine);
    }
    if opts.auto_connect {
        cmd.env("WEB_ACTION_AUTO_CONNECT", "1");
    }
    if let Some(idle) = opts.idle_timeout {
        cmd.env("WEB_ACTION_IDLE_TIMEOUT_MS", idle);
    }
    if let Some(timeout) = opts.default_timeout {
        cmd.env("WEB_ACTION_DEFAULT_TIMEOUT", timeout.to_string());
    }
    if let Some(cdp) = opts.cdp {
        cmd.env("WEB_ACTION_CDP", cdp);
    }
    if opts.no_auto_dialog {
        cmd.env("WEB_ACTION_NO_AUTO_DIALOG", "1");
    }
    if let Some(plugins) = opts.plugins {
        cmd.env("WEB_ACTION_PLUGINS", plugins);
    }
}
...
fn concurrent_daemon_config_error(session: &str) -> String {
    format!(
        "A daemon for session '{}' started concurrently with different daemon configuration. Retry the command so web-action can restart it with the requested configuration.",
        session
    )
}
        ...
        if path_len > 103 {
            return Err(format!(
                "Session name '{}' is too long. Socket path would be {} bytes (max 103).\n\
                 Use a shorter session name or set WEB_ACTION_SOCKET_DIR to a shorter path.",
                session, path_len
            ));
        }
    }
...
/// parse_command stamps with WEB_ACTION_DEFAULT_TIMEOUT when no explicit
...
mod tests {
    ...
    fn test_get_socket_dir_explicit_override() {
        let _guard = EnvGuard::new(&["WEB_ACTION_SOCKET_DIR", "XDG_RUNTIME_DIR"]);

        _guard.set("WEB_ACTION_SOCKET_DIR", "/custom/socket/path");
    ...
    fn test_get_socket_dir_ignores_empty_socket_dir() {
        ...
        _guard.set("WEB_ACTION_SOCKET_DIR", "");
        _guard.remove("XDG_RUNTIME_DIR");

        assert!(get_socket_dir()
            .to_string_lossy()
            .ends_with(".web-action"));
    }
    ...
    fn test_get_socket_dir_xdg_runtime() {
        ...
        _guard.remove("WEB_ACTION_SOCKET_DIR");
        _guard.set("XDG_RUNTIME_DIR", "/run/user/1000");

        assert_eq!(
            get_socket_dir(),
            PathBuf::from("/run/user/1000/web-action")
        );
    }
    ...
    fn test_get_socket_dir_home_fallback() {
        ...
        assert!(result.to_string_lossy().ends_with(".web-action"));
        assert!(
    ...
    fn test_get_socket_dir_namespace_scopes_base_directory() {
        ...
        _guard.set(
            "WEB_ACTION_SOCKET_DIR",
            "/tmp/web-action-test-sockets",
        );
        _guard.remove("XDG_RUNTIME_DIR");
        _guard.set("WEB_ACTION_NAMESPACE", "Worktree: One");

        assert_eq!(
            get_socket_dir(),
            PathBuf::from("/tmp/web-action-test-sockets")
                .join("namespaces")
                .join("worktree-one")
                .join("run")
        );
    ...
    fn test_walk_daemons_only_lists_current_namespace() {
        ...
        _guard.set("WEB_ACTION_SOCKET_DIR", dir.path().to_str().unwrap());
        _guard.remove("XDG_RUNTIME_DIR");
        ...
        _guard.set("WEB_ACTION_NAMESPACE", "one");
    ...
    fn test_spawn_race_loser_does_not_overwrite_winner_config() {
        let guard = EnvGuard::new(&["WEB_ACTION_SOCKET_DIR", "WEB_ACTION_NAMESPACE"]);
        let dir = tempfile::tempdir().unwrap();
    ...
    fn test_get_port_for_session() {
        let guard = EnvGuard::new(&["WEB_ACTION_NAMESPACE"]);
        guard.remove("WEB_ACTION_NAMESPACE");
    ...
    fn test_get_port_for_session_includes_namespace() {
        ...
        guard.set("WEB_ACTION_NAMESPACE", "Worktree: Two");
    ...
    fn test_daemon_version_matches_same_version() {
        ...
        let _guard = EnvGuard::new(&["WEB_ACTION_SOCKET_DIR", "XDG_RUNTIME_DIR"]);
        _guard.set("WEB_ACTION_SOCKET_DIR", dir.to_str().unwrap());
