...
pub(super) fn check(checks: &mut Vec<Check>) {
    ...
    let key_env = env::var("WEB_ACTION_ENCRYPTION_KEY").ok();
    ...
    if let Some(hex) = &key_env {
        if hex.len() == 64 && hex.chars().all(|c| c.is_ascii_hexdigit()) {
            checks.push(Check::new(
                ...
                category,
                Status::Pass,
                "WEB_ACTION_ENCRYPTION_KEY set (64-char hex)",
            ));
        } else {
            checks.push(
                Check::new(
                    ...
                    category,
                    Status::Fail,
                    "WEB_ACTION_ENCRYPTION_KEY is not a 64-char hex string",
                )
                .with_fix("export WEB_ACTION_ENCRYPTION_KEY=$(openssl rand -hex 32)"),
            );
        }
    ...
    if sessions_dir.exists() {
        let expire_days = env::var("WEB_ACTION_STATE_EXPIRE_DAYS")
            .ok()
        ...
        if total == 0 {
            ...
            checks.push(
                ...
                .with_fix(format!(
                    "web-action state clean --older-than {}",
                    expire_days
                )),
            );
        } else {
    ...
    if let Ok(policy_path) = env::var("WEB_ACTION_ACTION_POLICY") {
        ...
        if !p.exists() {
            checks.push(
                Check::new(
                    ...
                    Status::Fail,
                    format!(
                        "WEB_ACTION_ACTION_POLICY points to missing file: {}",
                        policy_path
                    ),
                )
                .with_fix("update or unset WEB_ACTION_ACTION_POLICY"),
            );
        } else {
