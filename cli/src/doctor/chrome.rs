...
pub(super) fn check(checks: &mut Vec<Check>) {
    ...
    if let Ok(engine) = env::var("WEB_ACTION_ENGINE") {
            ...
            if which_exists("lightpanda") {
                ...
                checks.push(
                    Check::new(
                        ...
                        category,
                        Status::Fail,
                        "WEB_ACTION_ENGINE=lightpanda but no lightpanda binary on PATH",
                    )
                    .with_fix("install lightpanda or unset WEB_ACTION_ENGINE"),
                );
            }
        }
    }
