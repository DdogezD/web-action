...
async fn collect_storage_in_target(
    ...
    client
        ...
        .await?;
    // NOTE: Intentionally skip Runtime.enable.
    // Runtime domain auto-enables on first evaluate call.
    // Avoiding the explicit enable prevents broadcasting
...
pub async fn save_state(
    ...
    if let Ok(key) = std::env::var("WEB_ACTION_ENCRYPTION_KEY") {
...
pub async fn save_auto_state_transactional(
    ...
    let final_path = if std::env::var("WEB_ACTION_ENCRYPTION_KEY").is_ok() {
...
fn read_state_json(path: &str) -> Result<String, String> {
    if is_encrypted_state(std::path::Path::new(path)) {
        let key = std::env::var("WEB_ACTION_ENCRYPTION_KEY").map_err(|_| {
            "Encrypted state file requires WEB_ACTION_ENCRYPTION_KEY".to_string()
        })?;
        let data =
...
/// Return the web-action state root (`~/.web-action`, falling back to
/// `<tempdir>/web-action` when the home directory can't be resolved).
...
pub fn get_state_dir() -> PathBuf {
    let base = if let Some(home) = dirs::home_dir() {
        ...
        std::env::temp_dir().join("web-action")
    };

    if let Ok(namespace) = std::env::var("WEB_ACTION_NAMESPACE") {
...
mod tests {
    ...
    fn test_state_show_nonexistent_file() {
        let result = state_show("/tmp/nonexistent-web-action-state-file.json");
        assert!(result.is_err());
    }

    #[test]
    fn test_state_clear_nonexistent_file() {
        let result = state_clear(Some("/tmp/nonexistent-web-action-state-file.json"));
        assert!(result.is_err());
    }
    ...
    fn test_state_clear_removes_transactional_backups() {
        let guard = crate::test_utils::EnvGuard::new(&["HOME", "WEB_ACTION_NAMESPACE"]);
    ...
    fn test_state_rename_nonexistent() {
        let result = state_rename("/tmp/nonexistent-web-action-state-file.json", "new-name");
        assert!(result.is_err());
    ...
    fn test_get_state_dir_namespace_scopes_sessions() {
        let _guard = crate::test_utils::EnvGuard::new(&["WEB_ACTION_NAMESPACE"]);
        _guard.set("WEB_ACTION_NAMESPACE", "Worktree: One");

        let dir = get_state_dir();
        let expected_state_suffix = PathBuf::from(".web-action")
            .join("namespaces")
            .join("worktree-one")
            .join("state");
