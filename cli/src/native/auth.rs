...
fn get_auth_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".web-action").join("auth")
    } else {
        std::env::temp_dir().join("web-action").join("auth")
    }
}
...
const ENCRYPTION_KEY_ENV: &str = "WEB_ACTION_ENCRYPTION_KEY";
const KEY_FILE_NAME: &str = ".encryption-key";

fn get_web_action_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
...
fn get_key_file_path() -> PathBuf {
    get_web_action_dir().join(KEY_FILE_NAME)
}
...
/// Read the encryption key from WEB_ACTION_ENCRYPTION_KEY env var or
/// ~/.web-action/.encryption-key file (matching the Node.js implementation).
...
fn ensure_encryption_key() -> Result<Vec<u8>, String> {
    ...
    let _ = writeln!(
        std::io::stderr(),
        "[web-action] Auto-generated encryption key at {} -- back up this file or set {}",
        key_file.display(),
        ENCRYPTION_KEY_ENV
    );
