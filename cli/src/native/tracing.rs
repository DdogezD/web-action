...
fn get_traces_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".web-action").join("tmp").join("traces")
    } else {
        std::env::temp_dir().join("web-action").join("traces")
    }
}

fn get_profiles_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".web-action").join("tmp").join("profiles")
    } else {
        std::env::temp_dir().join("web-action").join("profiles")
    }
}
