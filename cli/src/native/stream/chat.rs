...
const SKILL_NAMES: &[&str] = &["web-action", "slack", "electron", "dogfood", "agentcore"];
...
fn find_skills_dir() -> Option<std::path::PathBuf> {
    ...
    while let Some(d) = dir {
        ...
        if candidate.join("web-action").join("SKILL.md").exists() {
...
pub(crate) fn get_system_prompt() -> &'static str {
    ...
    PROMPT.get_or_init(|| {
        ...
        format!(
            r#"You are an AI assistant that controls a browser through web-action. You have an active browser session, but you can also create new sessions.

RULES:
- You MUST use the web_action tool for every browser action. NEVER claim you performed an action without calling the tool.
...
- Do not add `--json`.
- Do not run non-web-action programs.
...
- To create a new session: add `--session <name>` to any command (e.g. `web-action --session my-session open https://example.com`). If the session does not exist, it will be created automatically.
- To use a different browser engine: add `--engine <engine>` (e.g. `web-action --session lp-session --engine lightpanda open https://example.com`). Supported engines: chrome (default), lightpanda.

The following skill references describe web-action capabilities in detail. Use them when deciding which commands to run and how to approach tasks.
{sections}"#,
        )
    })
}

pub(crate) const CHAT_TOOLS: &str = r#"[{"type":"function","function":{"name":"web_action","description":"Execute an web-action command. Runs against the active session by default. Add --session <name> to target or create a different session, and --engine <engine> to choose a browser engine.","parameters":{"type":"object","properties":{"command":{"type":"string","description":"The command to execute, e.g. 'web-action open https://google.com' or 'web-action --session new-session open https://example.com' or 'web-action snapshot -i' or 'web-action click @e3'"}},"required":["command"]}}}]"#;
...
pub(crate) async fn execute_chat_tool(session: &str, command: &str) -> String {
    ...
    let stripped = single.strip_prefix("web-action ").unwrap_or(single);
    ...
    if !ALLOWED_COMMANDS.contains(&first_cmd) {
        return format!(
            "Blocked: '{}' is not a valid web-action command.",
            first_cmd
        );
    }
    ...
    cmd.args(&args)
        .env_remove("WEB_ACTION_DASHBOARD")
        .env_remove("WEB_ACTION_DASHBOARD_PORT")
        .env_remove("WEB_ACTION_STREAM_PORT");
