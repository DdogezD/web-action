...
const USER_AGENT_VALUE: &str = concat!("web-action/", env!("CARGO_PKG_VERSION"), " read");
...
mod tests {
    ...
    fn html_to_markdownish_keeps_header_after_head() {
        let html = r#"
          ...
          <article><h1>web-action</h1><p>Browser automation CLI.</p></article>
          </body></html>
        "#;
        let text = html_to_markdownish(html);
        assert!(text.contains("Home"));
        assert!(text.contains("# web-action"));
